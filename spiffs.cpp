    ...
    _formatIfMountFailed{formatIfMountFailed}
{
  if (_maxFilesOpen <= 0) {
    ESP_LOGE(TAG,
             "parameter maxFilesOpen must be greater than zero. got: %d",
             _maxFilesOpen);
...
void Spiffs::Register()
{
  ...
  conf.partition_label        = _partitionLabel.empty() ? NULL
  ...
  if (ret != ESP_OK) {
    if (ret == ESP_FAIL) {
      ESP_LOGE(TAG,
               "Failed to mount or format filesystem. (%s)",
               esp_err_to_name(ret));
    }
    else if (ret == ESP_ERR_NOT_FOUND) {
      ESP_LOGE(
        TAG, "Failed to find SPIFFS partition (%s)", esp_err_to_name(ret));
    }
    else {
...
{
  esp_vfs_spiffs_unregister(_partitionLabel.empty() ? NULL
                                                    : _partitionLabel.c_str());
}
